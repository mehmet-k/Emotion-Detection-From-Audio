...
def createMasterDataFrame(speaker_name,language,mod,ANGRY,HAPPY,NEUTRAL,SAD,SURPRISE):
    ...
    master_data_frame = []
    if ANGRY:
        data_angry = pd.read_csv('Angry'+mod+'.csv',header=None, index_col=0)
        #print("data angry:", data_angry.shape)
        master_data_frame = data_angry
    if HAPPY:
        data_happy = pd.read_csv('Happy'+mod+'.csv',header=None, index_col=0)
    ...
    if NEUTRAL:
        data_neutral = pd.read_csv('Neutral'+mod+'.csv',header=None, index_col=0)
    ...
    if SAD:
        data_sad = pd.read_csv('Sad'+mod+'.csv',header=None, index_col=0)
    ...
    if SURPRISE:
        data_surprise = pd.read_csv('Surprise'+mod+'.csv',header=None, index_col=0)
